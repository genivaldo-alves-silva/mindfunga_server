...
# a pasta filogenia já deve existir no servidor. Para acesso ao servidor veja no item 1, sobre cadastro...
...
## 2.3 Teste o .nex de exemplo e/ou converta teu .fas em .nex

Enviem o arquivo de exemplo para a tua pasta do servidor - [infile.nex](./MrBayes_Jobs/infile.nex).

- Converta teu .fas em .nex

```bash
# crie as pastas abaixo se ainda não existirem
cd filogenia/bayesiana

ls
# meu_arquivo.fas

python3 /opt/fasta2nexus/fasta2nexus.py ./meu_arquivo.fas ./convertido.nex
# cada item desta linha separado por espaço: motor script input output
```

## 2.4 Ao abrir o editor em mrbayes.sub, cole o conteúdo abaixo (ie.: Ctrl+Shift+V ou Ctrl+V).

Veja o arquivo de exemplo - [mrbayes.sub](./MrBayes_Jobs/mrbayes.sub).

```bash
# abra com o editor 'nano' e cole o conteúdo copiado abaixo
nano mrbayes.sub
```
...
arguments               = --mca plm isolated --oversubscribe -np 8 /usr/local/bin/mb /endereço/completo/do/infile.nex #ou o convertido.nex
...
## 2.6 O arquivo mrbayes.sub pode ser criado e editado no teu computador e ser transferido junto com o infile.nex
...
## 2.7 Acompanhando o Job, verificando resultados e download
...
## 2.8 Verificou que a corrida conclui ou você encontrou o arquivo .tre. Vamos transferir os arquivos necessários para o teu computador e verificar os arquivos .p no TRACER e a .tre no FIGTREE.
...
## Existem alternativas de manipulação de vários arquivos. Veja algumas possibilidades em [7. Manipulação de arquivos](./7.%20Manipulação%20de%20arquivos.md).
