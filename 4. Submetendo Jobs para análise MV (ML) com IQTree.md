...
- Para análises multiloci veja o item 4.8 Submetendo IQtree job multiloci.
...
- A primeira opção é com a [matriz multiloci](./ML_Jobs/IQTree/concatenada.fas) em um único arquivo. 
- A segunda opção são as várias regiões do DNA separas. !! Nesta segunda opção deve ter cuidado com o nome dos espécimes. Eles devem ser iguais em todas matrizes e quando o espécime x não tem dado para região y com z de tamanho, ele deve aparecer na lista mesmo que com - ou ? na quantidade correspondente ao tamanho da matriz.
- A terceira opção

## Para regiões com éxons e íntrons como TEF, precisa ser concatenado para poder subir o arquivo de partição, informando onde estão os íntrons e éxons. E quando éxons deve informar em códons (ex.).

## Para descobrir o valor de -T somente com a matriz concatenada e seguir o [item 4.5.1](#451-analisando-o-arquivo-de-input-fas-para-descobrir-quanto-recurso-computacional-será-necessário-e-será-alocado-no-servidor-para-você-nesta-análise-de-ml-com-raxml-ng).
...
executable              = /usr/local/bin/iqtree3
arguments               = -s concatenado.fas -p partitions_iqtree.txt -m MFP+MERGE -B 1000 -alrt 1000 -T 2
